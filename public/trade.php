<?
	...
	echo "howdy1!";
	$sql = sprintf("UPDATE users SET balance='%f' WHERE userid='%d'",
		        $_SESSION['balance'] + $_POST['quantity'] * $_POST['price'], $_SESSION['id']);
	$conn->exec($sql);// update balance in users table
	$sql = sprintf("UPDATE portfolio SET shares='%d' WHERE userid='%d' AND symbol='%s'",
	...
	$sql = sprintf("INSERT INTO history (userid, symbol, shares, sell_buy, price, cost, time) 
			VALUES ('%d', '%s', '%d', '0', '%f', '%f', '%s')",$_SESSION['id'], $_POST["symbol"], 
			$_POST["quantity"], $_POST["price"], $_POST["quantity"] * $_POST["price"], $today);
	$conn->exec($sql); // insert a record into table history
	}	
	else { // buy stocks
	    $sql = sprintf("SELECT shares FROM portfolio WHERE userid='%d' AND symbol='%s'", 
		...
		$conn->exec($sql);//update shares in portfolio table
		$sql = sprintf("INSERT INTO history (userid, symbol, shares, sell_buy, price, cost, time) 
				VALUES ('%d', '%s', '%d', '1', '%f', '%f', '%s')",$_SESSION['id'], 
				$_POST["symbol"], $_POST["quantity"], $_POST["price"], 
				$_POST["quantity"] * $_POST["price"], $today);
		$conn->exec($sql); // insert a record into table history
	    }
	    else { // buy new stocks
		$sql = sprintf("UPDATE users SET balance='%f' WHERE userid='%d'",
				$_SESSION['balance'] - $_POST['quantity'] * $_POST['price'],$_SESSION['id']);						
		$conn->exec($sql);// update balance in users table
		$sql = sprintf("INSERT INTO portfolio (userid, symbol, shares) VALUES ('%d', '%s', '%d')",
				$_SESSION['id'], $_POST['symbol'], $_POST['quantity']);							
		$conn->exec($sql);//update shares in portfolio table
		$sql = sprintf("INSERT INTO history (userid, symbol, shares, sell_buy, price, cost, time) 
				VALUES ('%d', '%s', '%d', '1', '%f', '%f', '%s')",
				$_SESSION['id'], $_POST["symbol"], $_POST["quantity"], $_POST["price"], 
				$_POST["quantity"] * $_POST["price"], $today);			
		$conn->exec($sql); // insert a record into table history
		}
	}
